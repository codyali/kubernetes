...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=liyakatsahil/multi-server:$SHA
kubectl set image deployments/client-deployment client=liyakatsahil/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=liyakatsahil/multi-worker:$SHA